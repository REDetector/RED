...
public class BasicFilter {
    ...
    public BasicFilter(DatabaseManager databaseManager,String rnaVcf,String specificTable, String basicTable) {
    ...
    public void createSpecificTable() {
        ...
        databaseManager.createTable(specificTable, "(chrome varchar(30)," + Utilities.getInstance().getS2() + "," +
                "index(chrome,pos))");
    }
    ...
    public void specificf() {
        ...
        databaseManager.insertClause("insert into " + specificTable + "(" + Utilities.getInstance().getS3() + ")  select * from " +
...
//        
//        databaseManager.executeSQL("create temporary table newtable select distinct * from " + specificTable);
//        databaseManager.executeSQL("truncate table " + specificTable);
//        databaseManager.executeSQL("insert into " + specificTable + " select * from  newtable");
...
//    }
    ...
    public void basicf(int qua, int num) {
        try {
            System.out.println("bfilter start" + " " + df.format(new Date()));// new Date()Ϊ��ȡ��ǰϵͳʱ��

            //create table and insert data into it
            databaseManager.deleteTable(basicTable);
            databaseManager.createTable(basicTable, "(chrome varchar(30)," + Utilities.getInstance().getS2() + ",index(chrome,pos))");
            ResultSet rs = databaseManager.query(specificTable, "chrome,pos,AD","1");
            ...
            for (int i = 0, len = coordinate.size(); i < len; i++) {
                switch (i % 3) {
                    ...
                    case 2:
                        ...
                        if (ref_n + alt_n > num) {
                            databaseManager.executeSQL("insert into " + basicTable + "( " + Utilities.getInstance().getS3() +
                                    ") (select * from " + specificTable + " where filter='PASS' and pos=" + ps
                                    + " and qual >" + qua + " and chrome='" + chr + "')");
                            count++;
                            if(count%10000==0)
