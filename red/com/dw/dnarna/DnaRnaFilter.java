...
public class DnaRnaFilter {
	...
	public DnaRnaFilter(DatabaseManager databaseManager,String dnaVcf,String DnaRnaTable,String refTable){
	...
	public boolean esdr() {
			...
			databaseManager.createTable(DnaRnaTable, "(chrome varchar(30)," + Utilities.getInstance().getS2() + ")");
	...
	public void dnarnaFilter(){
		try {
			System.out.println("df start"+" "+df.format(new Date()));// new Date()为获取当前系统时间
				...
				ResultSet rs=databaseManager.query(dnaVcf, "chrome", "1 limit 0,1");
			...
			for (int i = 0, len = coordinate.size(); i < len; i++) {
				if (i % 2 == 0) {
					...
					rs=databaseManager.query(dnaVcf, "GT", "chrome='"+chr+"' and pos="+ps+"");
					while (rs.next()) 
					{
							if(bool)
							{
							chr="chr"+chr;
							databaseManager.executeSQL("insert into "+DnaRnaTable+" select * from "+refTable+" where chrome='"+ chr+ "' and pos="+ ps+ "");
							count++;
                            if(count%10000==0)
