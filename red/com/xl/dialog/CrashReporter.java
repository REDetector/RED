...
public class CrashReporter extends JDialog implements ActionListener {
	...
	// public static final String reportURLString =
	// "http://bilin1/cgi-bin/public/crashreporter.cgi";
	...
	private class ReportSender extends JDialog implements ActionListener,
		...
		private String makeReportText() {
			...
			if (e.getClass().getName().indexOf("RED") >= 0) {
				foundREDClass = true;
			}
			sb.append("\n");
			sb.append(e.getMessage());
			if (e.getMessage() != null
					&& e.getMessage().indexOf("RED") >= 0) {
				foundREDClass = true;
			}
			sb.append("\n\n");
			...
			for (int i = 0; i < elements.length; i++) {
				...
				sb.append("\n");
				if (elements[i].toString().indexOf("RED") >= 0) {
					foundREDClass = true;
				}
			}
