...
/**
 * Paired Data Set is used for HiC type experiments. It's not intended for use
 * for normal paired end reads where the pairs represent part of a continuous
...
public class PairedDataSet extends DataSet implements HiCDataStore {
    ...
    /**
     * This is the resolution of the length corrections we're going to apply to
     * HiC data.
     */
    ...
    public PairedDataSet(String name, String fileName,
		...
		/*
         * Because our duplicate removal will be based on HiC duplication (ie
		 * both ends are the same) we always set the superclass flag to be false
		 * and then we only pass on the reads which are deduplicated. This also
		 * means we need to store the actual remove duplicates flag value here
		 * so we don't rely on the one from the superclass.
		 */
    ...
    /**
     * This method is called if a sequence is read from the input file but
     * encounters an error which causes it to not be imported. If we are holding
     * on to one sequence of a pair then we need to discard that and return
     * false. If we're not then we return true to tell them to skip the next
     * line
     */
    ...
    public void addData(String c, SequenceRead read) {
        addData(c, read, false);
    }

    /**
     * This method is used to add data to the paired data set and should be
     * called by all parsers which create a new set. Pairs of reads should be
     * added sequentially to the data set and will be paired by it internally.
     * <p/>
     * Only the SeqMonk parser should ever set the noReverse parameter to true.
     * Specifying this for all data lets the dataset skip the sorting step when
     * caching which is otherwise really slow, but if this is incorrectly
     * skipped then subsequent results returned by the data set will be wrong.
     *
     * @param c
     * @param read
     * @param skipSorting
     */
    public void addData(String c, SequenceRead read, boolean skipSorting) {
        ...
        // Skip cis reads which are too close together
        else if (c == lastChromosome
                && SequenceReadUtils.fragmentLength(lastRead, read) < minDistance) {
            ...
            if (lastChromosome != c) {
                ...
                if (transChromosomeCounts.containsKey(lastChromosome)) {
                    transChromosomeCounts.put(lastChromosome,
                            transChromosomeCounts.get(lastChromosome) + 1);
                } else {
                    transChromosomeCounts.put(lastChromosome, 1);
                }
                if (!skipSorting) {
                    if (transChromosomeCounts.containsKey(c)) {
                        transChromosomeCounts.put(c,
                                transChromosomeCounts.get(c) + 1);
                    } else {
                        transChromosomeCounts.put(c, 1);
                    }
                }
            } else {
                ...
                if (cisChromosomeCounts.containsKey(c)) {
                    cisChromosomeCounts.put(lastChromosome,
                            cisChromosomeCounts.get(c) + increment);
                } else {
                    cisChromosomeCounts.put(c, increment);
                }
            }

            // System.err.println("Keeping read pair from "+c+" and "+lastChromosome+" with distance "+(Math.max(SequenceRead.start(read),
            // SequenceRead.start(lastRead))-Math.min(SequenceRead.end(read),SequenceRead.end(lastRead)))+" start is "+SequenceRead.start(read)+" end is "+SequenceRead.end(read)+" start2="+SequenceRead.start(lastRead)+" end2="+SequenceRead.end(lastRead));
            ...
            try {
                ...
                readData.get(lastChromosome).hitCollection.addHit(c, lastRead,
                        read);
                ...
                if (!skipSorting) {
                    ...
                    readData.get(c).hitCollection.addHit(lastChromosome,
                            read, lastRead);
                }

            } catch (REDException sme) {
    ...
    private synchronized HiCHitCollection updateCacheToChromosome(String c) {
        ...
        if (lastCachedHits == null
                || !c.equals(lastCachedHits.getSourceChromosomeName())) {
			 ...
			 * start. This is based on there being an even distribution of reads
			 ...
			 * actually looked up. If the next request is close to the last one
			 * then we start from the last position instead. This works because
			 * most sets of requests come from quantitation methods and usually
			 * operate sequentially along the chromosome.
			 * 
			 * We also make the assumption that no read is >5kb in length (which
			 * we can easily modify later if we have to) otherwise we'd have to
			 * start from the beginning of the chromosome each time.
			 */
    ...
    public HiCHitCollection getHiCReadsForProbe(Probe p) {
        ...
        HiCHitCollection hitCollection = new HiCHitCollection(p.getChr());

        HiCHitCollection lastCachedHits = updateCacheToChromosome(p.getChr());

        // int hitCount = 0;

        if (lastChromosomeHitNames.length != lastIndices.length) {
            throw new IllegalArgumentException("Chr names "
                    + lastChromosomeHitNames.length + " but indices "
                    + lastIndices.length);
        }

        for (int c = 0; c < lastChromosomeHitNames.length; c++) {

            SequenceRead[] sourceReads = lastCachedHits
                    .getSourcePositionsForChromosome(lastChromosomeHitNames[c]);
            SequenceRead[] hitReads = lastCachedHits
                    .getHitPositionsForChromosome(lastChromosomeHitNames[c]);
            ...
            // of the hit collection so that this can't happen and we're
            // synchronized
            ...
            if (sourceReads.length != hitReads.length) {
                throw new IllegalStateException(
                        "Source reads and hit reads had different lengths for chr "
                                + lastChromosomeHitNames[c]
                                + " "
                                + sourceReads.length
                                + " vs "
                                + hitReads.length
                                + " current values are "
                                + lastCachedHits
                                .getSourcePositionsForChromosome(lastChromosomeHitNames[c]).length
                                + " and "
                                + lastCachedHits
                                .getSourcePositionsForChromosome(lastChromosomeHitNames[c]).length);
            }
            ...
            for (int i = startPos - 1; i >= 0; i--) {
                ...
                if (sourceReads[i].getEnd() < p.getStart() - 5000) {
                    // System.err.println("Finished backtracking at index "+i+" position "+SequenceRead.midPoint(sourceReads[i]));
                    break;
                }

                if (SequenceReadUtils.overlaps(sourceReads[i], p)) {
                    // They overlap.
                    // ++hitCount;
                    hitCollection.addHit(lastChromosomeHitNames[c],
                            sourceReads[i], hitReads[i]);
            ...
            for (int i = startPos; i < sourceReads.length; i++) {
                // Reads come in order, so we can stop when we've seen enough.
                if (sourceReads[i].getStart() > p.getEnd()) {
    ...
    public synchronized HiCHitCollection getHiCReadsForChromosome(String c) {
        ...
        if (readData.containsKey(c)) {
            ...
            if (readData.get(c).hitCollection != null) {
                ...
                if (lastCachedHits != null
                        && lastCachedHits.getSourceChromosomeName() == c) {
                    return lastCachedHits;
                }

                // Signal that we're accessing the cache so the cache icon can
                // blink!
                ...
                try {
                    ...
                    lastChromosomeHitNames = lastCachedHits
                            .getChromosomeNamesWithHits();
            ...
            lastCachedHits = new HiCHitCollection(c);
    ...
    public synchronized HiCHitCollection getExportableReadsForChromosome(
            String c) {

        if (!isFinalised)
            finalise();

        HiCHitCollection exportableReads = new HiCHitCollection(c);
        ...
        for (int i = 0; i < chrs.length; i++) {
            ...
            // We only include data for chromosomes the same or later than our
            // own

            Chromosome thisChr = REDApplication.getInstance().dataCollection()
                    .genome().getChromosome(chrs[i]);

            if (c.compareTo(thisChr.getName()) < 0) {
                System.err.println("Ignoring " + thisChr + " when exporting "
                        + c);
                continue;
            }

            SequenceRead[] source = redundantReads
                    .getSourcePositionsForChromosome(chrs[i]);
            SequenceRead[] hits = redundantReads
                    .getHitPositionsForChromosome(chrs[i]);

            if (c != thisChr.getName()) {
                // Put everything from this chromosome into the results
                for (int j = 0; j < source.length; j++) {
                    exportableReads.addHit(chrs[i], source[j], hits[j]);
                }
            } else {
                // Put only the entries where the hit is later than the source
                // in
                for (int j = 0; j < source.length; j++) {

                    // We include only hits where the hit is greater than the
                    // source
                    // to avoid repetition. If the two are the same then we take
                    // half
                    // of the hits based on the current index.

                    if ((source[j] == hits[j] && j % 2 == 0)
                            || SequenceReadUtils.compare(source[j], hits[j]) >= 0) {
                        exportableReads.addHit(chrs[i], source[j], hits[j]);
                    }
                }
            }
    ...
    public synchronized float getCorrectionForLength(String c, int minDist,
                                                     int maxDist) {
        ...
        float adjustedCorrection = totalCorrection
                / (((endIndex - startIndex) + 1));

        // if (adjustedCorrection <
        // cds.getCorrectionForIndex(c.length()/DISTANCE_GROUP_LENGTH) ||
        // adjustedCorrection > cds.getCorrectionForIndex(0)) {
        // throw new
        // IllegalArgumentException("Correction "+adjustedCorrection+" was out of bounds of actual corrections indices were "+startIndex+" and "+endIndex+" total correction was "+totalCorrection);
        // }
    ...
    public synchronized void finalise() {
        ...
        // Finally, now that we've sent all of the correct data up to the
        // superclass
        ...
        System.err.println("HiC finalise="
                + ((finaliseEndTime - finaliseStartTime) / 1000d)
                + " super submit="
                + ((superSubEndTime - finaliseEndTime) / 1000d)
                + " super finalise="
                + ((superFinaliseEndTime - superSubEndTime) / 1000d));
    ...
    private class ChromosomeDataStore implements Runnable {
        ...
        public ChromosomeDataStore(String c) {
            hitCollection = new HiCHitCollection(c);
            int chrLength = REDApplication.getInstance().dataCollection()
                    .genome().getChromosome(c).getLength();
            distanceCorrections = new float[getIndexForLength(chrLength) + 1];
        }
        ...
        public void run() {
            ...
            if (needToSort) {
                ...
                // We still need to trim the long vectors we've used to store
                // data. This
                // happens implicitly when sorting, but here we need to be
                // explicit.
                hitCollection.trim();
            }
            ...
            // We need to do distance counts for cis chromosomes only, but we
            // need to get total
            // counts for all chromosomes, so we need to work our way through
            // the whole lot

            String[] chromosomeNames = hitCollection
                    .getChromosomeNamesWithHits();
            ...
            for (int c = 0; c < chromosomeNames.length; c++) {
                ...
                SequenceRead[] sourceReads = hitCollection
                        .getSourcePositionsForChromosome(chromosomeNames[c]);
                SequenceRead[] hitReads = null;

                boolean sameChromosome = chromosomeNames[c]
                        .equals(hitCollection.getSourceChromosomeName());

                if (sameChromosome) {
                    hitReads = hitCollection
                            .getHitPositionsForChromosome(chromosomeNames[c]);
                }

                for (int i = 0; i < sourceReads.length; i++) {
                    if (sameChromosome) {
                        int distance = (int) SequenceReadUtils.fragmentLength(
                                sourceReads[i], hitReads[i]);
                        ++distanceCounts[getIndexForLength(distance)];
                    }

                    minMaxLength.addValue(sourceReads[i].length());

                    // Add this length to the total
                    readLengths += sourceReads[i].length();

                    // Increment the appropriate counts
                    totalReads++;
                    ;
                    if (sourceReads[i].getStrand() == Strand.POSITIVE) {
                        forwardReads++;
                    } else if (sourceReads[i].getStrand() == Strand.NEGATIVE) {
                        reverseReads++;
                    } else {
                        unknownReads++;
                    }
            ...
            System.err.println("Found " + totalReads + " reads on "
                    + hitCollection.getSourceChromosomeName());
            ...
            // We now need to correct for the number of possible distance
            // combinations
            ...
            for (int i = 0; i < distanceCounts.length; i++) {
                randomReadDistribution[i] = ((float) (distanceCounts.length - i))
                        / totalCategoryCount;
            }

            // Now work out the average number of interactions we expect to
            // observe
            ...
            // Now we work out a specific correction factor for the actual data
            // in this
            // dataset. We need to compare the actual observations we get with
            // the
            ...
            for (int i = 0; i < distanceCounts.length; i++) {
                ...
                // Work out the proportion of reads which actually fell into
                // this size range
                float observedProportion = ((float) distanceCounts[i])
                        / totalObservations;

                // Work out the ratio to what we should have seen from a random
                // distribution
                float distanceCorrection = observedProportion
                        / randomReadDistribution[i];

                // If we don't have enough data to assign a sensible value then
                // use the last
                // group where we did.
                if ((distanceCounts[i] < 10) && lastDecentCorrection > 0) {
                    // We don't really have enough data to be able to correct
                    // this
