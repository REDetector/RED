...
public class Query {
    ...
    public static Vector<SiteBean> queryAllEditingInfo(String tableName) throws SQLException {
        return queryAllEditingInfo(tableName, null, null, null);
    }
     ...
     * @return A collection which contains all information about a site.
     */
    public static Vector<SiteBean> queryAllEditingInfo(String tableName, String[] columns, String selection,
        String[] selectionArgs) throws SQLException {
        Vector<SiteBean> siteBeans = new Vector<SiteBean>();
        ResultSet rs = databaseManager.query(tableName, columns, selection, selectionArgs);

        while (rs.next()) {
            SiteBean p = new SiteBean(rs.getString(1), rs.getInt(2), rs.getString(3), rs.getString(4).charAt(0),
                rs.getString(5).charAt(0), rs.getFloat(6), rs.getString(7), rs.getString(8), rs.getString(9),
                rs.getInt(10), rs.getInt(11), rs.getString(12).charAt(0), rs.getString(13).charAt(0), rs.getFloat(14),
                rs.getFloat(15), rs.getFloat(16));
            siteBeans.add(p);
        }
        return siteBeans;
    }
    ...
    public static Site queryEditingSite(String tableName, String chrom, int pos) throws SQLException {
        ResultSet rs = databaseManager.query(tableName, new String[] { "chrom", "pos", "ref", "alt" },
            " chrom=? AND pos=?", new String[] { chrom, pos + "" });
        if (rs.next()) {
            return new Site(rs.getString(1), rs.getInt(2), rs.getString(3).charAt(0),
    ...
    public static Vector<Site> queryEditingSitesForChr(String tableName, String chrom) throws SQLException {
        Vector<Site> siteVector = new Vector<Site>();
        ResultSet rs = databaseManager.query(tableName, new String[] { "chrom", "pos", "ref", "alt" }, " chrom=?",
            new String[] { chrom });
        while (rs.next()) {
            Site p = new Site(rs.getString(1), rs.getInt(2), rs.getString(3).toCharArray()[0],
    ...
    /**
     * Retrieve a SiteSet from database by a given sample name. We first query all table names from database and select
     * all tables relative to this sample. Then we initiate the root of this site set model (i.e., the SiteSet) using
     * the sample RNA VCF file. Next, we retrieve the leaves and the leaf params of this tree according to the table
     * name.
     * <p/>
     * For example, there is a sample named BJ22. Tables in database are BJ22_rnavcf, BJ22_rnavcf_etfilter_A_G,
     * BJ22_etfilter_qcfilter_20_6, BJ22_qcfilter_rrfilter. First, we regard BJ22_rnavcf as a root of the tree model,
     * then the link between rnavcf and etfilter can be seen by BJ22_rnavcf_etfilter_A_G table, and its parameter is
     * A->G. So we can retrieve a leaf of the root as 'ALl Sites'->'Focus on A to G'. As a result, we can retrieve all
     * leaves as the following tree model:
     * <p/>
     * BJ22_rnavcf
     * <p/>
     * - Focus on A to G
     ...
     * @throws RedException If table name has been renamed or deleted in database, and the method can not find them,
    ...
    public synchronized static SiteSet getSiteSetFromDatabase(String sampleName) throws RedException, SQLException {
        ...
        SiteSet siteSet = new SiteSet(sampleName, "The original RNA editing sites from RNA VCF file.",
            queryAllEditingSites(rnaVcf).toArray(new Site[0]));
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (String section : sections) {
                if (section.contains(DatabaseManager.FILTER)
                    || section.contains(DatabaseManager.RNA_VCF_RESULT_TABLE_NAME)) {
                    if (count == 0) {
            ...
            if (previousFilter == null || currentFilter == null) {
                throw new RedException(
                    "Unknown filter table:" + tableName + ", please don't rename the filter name in the database.");
            }
